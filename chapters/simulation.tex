...
This allows researchers to compare the physical readouts they can observe to predicted readouts.
Finally, using the phase spaces outside of the desired signal,
minor corrections to the simulation must be made.
Simulated energies from the detector model might not be the exact same as
what the physical detector produces, for example,
and they must be made to match to make the signal analysis clean.
...
show how the two respective processes can result in the same final state as the signal process.
...
After final state particles are predicted, they take a shower.
