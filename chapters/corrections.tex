...
\subsection{LO to NLO Reweighing}
...
\item $b$-quarks are not in the matrix element, but are produced in the parton shower step
...
but they are only generated for $p_{T,V} > \SI{100}{GeV}$.
...
However, expected events with no $b$-quarks and events with $p_{T,V} < \SI{100}{GeV}$
...
is to generate NLO samples and reweigh the LO sample
...
are used to reweight the LO simulation as a function of $p_{T,V}$.
\begin{itemize}
...
\end{itemize}
The change to the $p_{T,V}$ spectra and the resulting improved agreement between data and MC
are shown in Figure~\ref{fig:nlo-reweigh}.
\begin{figure}
  \centering
  ...
  \caption[LO to NLO reweighing shape comparisons]{
    The shapes for $p_{T,V}$ in the V+light control regions are shown for 2017.
    The left plots are before LO to NLO reweighing,
  ...
  \label{fig:nlo-reweigh}
\end{figure}

\subsection{Vector Boson Transverse Momentum}

\subsection{Electroweak Corrections to Signal Process}
