...
bins. \phil{Is it really 0.1 per bin, this seems high?,  I would have
  expected an order of magnitude lower. Integrated over all bins, I
  recall we had it under 1 percent.  }
...
less than 2.5. \phil{I am confused by this cut, do mean MET triggers
  from the electron mismeasurement, or at least influenced by the electron?}
...
\begin{figure}
  ...
  \caption[MET trigger scale factors]{
    \phil{label y-axis, say MET Trigger scale factor too, I was
      confused. Also mention how the uncertaintiy is computed given
      the two lines.}
...
\end{gather}
\phil{Is it not the magnitude of the right formula, since there is a
  direct sum}
...
\phil{I understtand NLO/LO reweighting for inclusive production, andI
  think what youare doing is fine, but for z+b production, I am
  confused what is done. Are you reweighting? If so are you computing
  a k-factor with an NLO b-sample, what is you choice of b-generation,
  do you use 5-flavor pdfs, this should be specified.}
\begin{itemize}
...
\end{gather}
\phil{Pleasde define all of the terms in teh formula, I dont't know
  what $sigma_{t-loop}$ is even. }
...
\begin{figure}
  ...
  \caption[]{ \phil{ where is the caption??}
  }
...
\item QCD scale uncertainties for the signal production cross section
  ...
  bins. \phil{why do you say for each bin, is this to imply some
    correlation model?}
...
\item The MET trigger efficiencies are also applied as a normalization uncertainty
  to the 0-lepton regions. \phil{you show shape uncertainties before,
    please explain why you don't apply shape}
\end{itemize}
...
\item The energy scale is varied by varying all jet energies up and down by
  one standard deviation. \phil{for this can you put a reference}
\item The $b$-tagging efficiency uncertainties are calculated
  ...
  so they each affect a different subset of events when
  applied. \phil{do you have a plot of the uncertainties for b-tagging
    it would be good to see. Also, can you explain why its
    uncorrelated, at minimum put a reference}
\item The $b$-tagging for fat jets is applied for two different working points,
  medium and tight, and is binned in $p_T$.
  These working points and $p_T$ bins are decorrelated from each
  other. \phil{ please explain why its uncorrelated. In fact, I think
    this b-tagging deserves a sub sectin in this chapter where you
    explainhow these uncertiainties are derived. 
...
\phil{ This section is pretty light on details, I really think you
  should elaborate on a few things, I have pointed them out along the
  way. In particular, I think you should put a page or two on the
  b-tagging uncertainties. Additionally, you don't talk about the
  uncertainties in the boosted category (double-b) and how they are
  different. } 
