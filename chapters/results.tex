...
\begin{figure}
  ...
  \caption[Control region plots for two electrons in 2017]{
    ...
    The left column shows the low $p_T(V)$ bin of 75 to \SI{150}{GeV},
    and the right column shows the medium $p_T(V)$ bin of 150 to \SI{250}{GeV}.
    \phil{Could you cmoment in the text on the NLO correction; from
      what I read its only on the $p_{T}$ and not on jet distributions
      like the sub leading jet, this will impact that high boson pT
      and low sub leading jet pT(bottom right figure). }
  }
...
\begin{figure}
  ...
  \caption[Control region plots for two muons in 2018]{
    ...
    \phil{I am having a lot of trouble reconciling the chi2 values
      with what is shown in the plots. Furthermore why is the
      normalization so different from teh 2017, I hope you comment on
      this in the text. } 
  }
...
\phil{What is the loss function?}
...
to that of the observed data. \phil{You need a paragraph talking about
  what unblinding means, there are many ways to define this. The Higgs
  approach is specific to the HIggs group, B2G for example has a very
  different unblinding procedure. } 
The distributions of a the values from the set of toys allows the evaluation of the $p$-value
of the test statistic observed in data. \phil{of the values} 
...
they were first tested on $V\!Z$. \phil{ I think this section could
  use an intro section explaining the strategy towards getting the
  results. } 

\subsection{$V\!Z$ Cross Check Analysis}

The $V\!Z$ cross check analysis mostly duplicated the $V\!H$. \phil{This needs more than one
  sentence, also your sentence is not grammatically correct}
...
Figure~\ref{fig:vz-gof}. \phil{You need a reference for GOF, use Bob
  Cousins' paper on this}
...
uncertainties. \phil{Do you have the GOF without systematics, or
  reducing the systematics, it would be good to show in your thesis as
  a justification of this explanation. Such a low GOF is worrisome for
me otherwise. } 
...
analysis.  \phil{The caculation of 486 should go in your talk, I am
  struggling to get to that
  number. $486/3years/3finalstates/6categories?=9$ $9$ seems to high
...
decreasing the overall possible variation. \phil{This explanation does
  not make sense to me. If you have two nuisances that are
  uncorrelated, they remain uncorrelated throughout the fit. If they
  are both describing the same thing they will each get constrained so
  that the sum of their uncertainties in quadrature is equal to the
  data. Thinking through, I think I knwo what you are saying, but you
...
\begin{figure}
  ...
  \caption[$Z\rightarrow \mu\mu$ $V\!Z$ distributions for low $p_T$ in 2017]{
    ...
    The bottom row shows the signal region. \phil{ Can you make the
      signal region   plot aggregated over the years, ideally weighted by their
      signifcance contrbituion, also can you make it log so that we
      see the VZ contribution lastly can you make the VZ signal the
      redline not the VH}
  }
...
\begin{figure}
  ...
  \caption[$W\rightarrow e\nu$ $V\!Z$ distributions for the boosted selection in 2018]{
    ...
    > \SI{400}{GeV}$.
    \phil{same comments as before except for the two signal regions in
    the bottom two rows.} 
  }
...
\begin{figure}
  ...
  \caption[Measured STXS values of $V\!Z$]{
    ...
    \phil{Can you show the ggZH and qqZH split as well. Also, the WZ
      vs ZZ 2D contour, it seems that the scale factors are quite different}
  }
  \label{fig:vz-stxs}
\end{figure}
\phil{You should show the WZ and ZZ cobmined results, also do you have
  an explanation why WZin particular is so off? It looks like 2.5 sigma}

\subsection{$V\!H$ Combination Fit Results}

\phil{big comment about this section is you should make some summary
  distributions combining sthe signal acrss years ideally (weighted
  by signficance in fit). Also all of your DNN plots need to be log, I
can't see the signal otherwise. }
...
\begin{figure}
  ...
  \caption[$Z\rightarrow ee$ $V\!H$ distributions for medium $p_T$ in 2018]{
    ...
    \phil{where is the highest DNN score. If it fell of the plt adjust
      the range. }
  }
...
\begin{figure}
  ...
  \label{fig:vh-stxs}
\end{figure}
\phil{given how weird the results are have you tried injecting other
  Higgs mass points? }
...
=======
\phil{This chapter really does feel incomplete. You need to comment
  about the elephant in the room (large and systematic
  disagreement). Othwerise, it reads really weird. There is nothing
  evidently incorrect with this approach, so you should stand by your
  results, and be very clear that things are notn consistent with the SM. } 
>>>>>>> phil/phil
