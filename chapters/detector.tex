...
The $b$ quarks also hadronize to form color singlets well before reaching the detector,
...
and $b$ hadrons have a relatively long lifetime due to small values of $V_{cb}$ and $V_{ub}$.
...
This imbalance is labeled Missing Transverse Energy, $E^\textrm{miss}_T$, or MET.
...
The muon and silicon trackers reach up to $|\eta| < 2.5$,
...
One key design feature of the silicon detector is that it is non-destructive.
...
The two calorimeters are destructive because they absorb the particles that interact with them
...
\begin{figure}
  ...
  \caption[CMS detector slice]{
    ...
    The four detector layers are labeled and show the penetration
...
A key feature of CMS is the solenoid which generates a strong magnetic field throughout the detector.
...
Outside of the pixel detector layers are the strip detectors.
Silicon strip detectors that use coincident signals to
more precisely locate the locations of charged tracks.
...
About 1\% of these strips are triggered each bunch crossing.

The active material of the silicon detector is semi-conducting silicon.
...
Because of this, the silicon detector cannot detect neutral particles,
...
Layers of muon chambers act much like the silicon tracking detectors,
...
This overview follows the overview in reference
\cite{Sirunyan_2017}.
...
while traveling through the magnetic field of CMS.
...
linked to a track in the inner silicon detector.
...
A dedicated conversion finder is used to identify pair production within the inner tracking detector
...
like $K_S$ or $\Lambda$ within the silicon detector,
...
Luckily, most collisions result in processes that are well understood,
so the corresponding additional data is not needed.
...
Each process looks slightly different in our signal selection.
...
showering calculation \cite{Frixione_1996}.
...
using a framework written in the Unified Modeling Language \cite{Lefebure:687188}.
...
Since the data is event-based, they are stored in n-tuple format \cite{Blomer_2018}
supported by ROOT \cite{brun1997root}.
