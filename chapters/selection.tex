...
\subsection{Isolated Electrons} \label{sec:ele-def}
...
Loose jet identification, based on the constituents,
...
\begin{table}
  ...
  \caption[DeepCSV working points]{
    ...
    The working points are defined by their mistag rates in simulation.
  }
...
larger jets, labeled fat jets, are used.
...
also labeled $E_T^\mathrm{miss}$ or MET,
...
or by one of the pictured leptons traveling out of the detector without being observed.
...
The region is enriched with light flavor jets, labeled $udsg$ for
...
A summary of requirements for each region in each channel
...
\begin{figure}
  \centering
  \begin{tikzpicture}
    [node distance=1cm, every edge/.style={arrow}]
    \node (start) [async] at (0,0) {Start \\ 1-lepton events. \\ $p_T(V) > \SI{150}{GeV}$? \\
      For both jets \\ $p_{T,j} > \SI{25}{GeV}$? \\ Minimum $b$-tag \\ passes loose? \\
      Di-jet $p_{T,jj} > \SI{100}{GeV}$? \\
      Di-jet $50 < m_{T,jj} < \SI{250}{GeV}$?};
    \node (checkmax) [goodstep] at (6, -1) {Maximum $b$-tag \\ passes medium?};
    \draw [->, thick] (start) -- node [font=\scriptsize, above] {Yes to all} (checkmax);
    \node (checkdphilight) [goodstep] at (3, -3) {$\Delta\phi(jj, V) > 2.5$?};
    \draw [->, thick] (checkmax) -- node [font=\scriptsize, above] {No} (checkdphilight);
    \node (light) [donestep, right=of checkdphilight] {$W$ + light CR};
    \draw [->, thick] (checkdphilight) -- node [font=\scriptsize, above] {Yes} (light);
    \node (reject) [badstep]  at (2, -7) {Reject event};
    \draw [->, thick] (start) -- node [font=\scriptsize, left] {No to one or more} (reject);
    \draw [->, thick] (checkdphilight) -- node [font=\scriptsize, right] {No} (reject);
    \node (numjet) [goodstep, right=of light] {$N_{aj} \le 1$};
    \draw [->, thick] (checkmax) -- node [font=\scriptsize, above] {Yes} (numjet);
    \node (tt) [donestep, below=of light] {$t\bar{t}$ CR};
    \draw [->, thick] (numjet) -- node [font=\scriptsize, below] {No} (tt);
    \node (checkdphiheavy) [goodstep, below=2cm of numjet] {$\Delta\phi(jj, V) > 2.5$?};
    \draw [->, thick] (numjet) -- node [font=\scriptsize, right] {Yes} (checkdphiheavy);
    \draw [->, thick] (checkdphiheavy) -- node [font=\scriptsize, above] {No} (reject);
    \node (checkm) [goodstep, below=of checkdphiheavy] {$90 < m_{jj} < \SI{150}{GeV}$?};
    \draw [->, thick] (checkdphiheavy) -- node [font=\scriptsize, right] {Yes} (checkm);
    \node (sig) [donestep, below=of checkm] {Signal Region};
    \draw [->, thick] (checkm) -- node [font=\scriptsize, right] {Yes} (sig);
    \node (heavy) [donestep, left=of sig] {$W$ + heavy CR};
    \draw [->, thick] (checkm) -- node [font=\scriptsize, above] {No} (heavy);
    
  \end{tikzpicture}
  \caption[A flowchart of 1-lepton regions]{
    Above a flowchart shows the process of using selection requirements to
    categorize events into the signal region or different control regions.
    The example is of the 1-lepton resolved channel, as detailed in Table~\ref{tab:resolved}.
    The other channels follow similar patterns.
  }
  \label{fig:w-category-flowchart}
\end{figure}
...
\begin{figure}
  ...
  \caption[MET $\phi$ distribution before and after shutting off HCAL modules]{
    ...
    is caused by mismeasuring the momenta of forward jets in that region.
  }
  \label{fig:met-peak}
\end{figure}
...
The signal region uses the same $b$-tag and mass value requirement as the other two channels.
