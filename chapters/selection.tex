...
from the analysis are given. \phil{rewrite this sentence, it sounds
  like German translated to english directly, its not english}
After that, selection requirements used to classify events
into different decay channels of the vector boson are specified. \phil{ditto}
...
would not include the corresponding particle. \phil{I am struggling to
...
\item They must have a higher transverse momentum at $p_T > \SI{25}{GeV}$.
  ...
  only one muon only needs to satisfy this $p_T$
  requirement. \phil{muon only needs=> muon needs}
  Only $p_T > \SI{15}{GeV}$ is required of the second muon. \phil{Only
    => A}
...
Any jet within $\Delta R < 0.4$ from a pre-selected lepton is
removed.\phil{You should forward reference the selections, even though
  I don't like foward referencing, wait maybe you don't do that in
  which case you should reference these ids with a citation}
...
\begin{table}
  ...
  \caption[DeepCSV working points]{
    ...
    rates. \phil{mis-tag rates of what data, simulation?}
  }
...
\cite{collaboration2019deep}. \phli{Can you list the training targets,
  or specify the loss definitin?}
...
$m_\mathrm{SD} > \SI{50}{GeV}$. \phil{ I actually don't know but I
  would presume you guys are using PUPPI jets, since it is the
  default, can you cite PUPPI please}
...
An additional technique to select interesting events with a large MET
\phil{with large MET}
is to compare the particle flow MET to other calculations of MET.
This analysis also uses track MET, or track MET. \phil{ I am seeing
  double, please fix}
...
\phil{Not your fault, but I don't think we can really simulate these
  variables well, its a bit of a shock that they use this}
...
and it is assumed that the MET in the event is 0. \phil{with what resolution?}
...
\begin{table}
  ...
  \caption[Summary of resolved selection cuts]{
    ...
    required to fail the relevant $b$-tagging criteria. \phil{minor
      point, cuts is jargon. Bigger point, I really think you need a
      diagram for these control regions its really hard to fllow otherwise}
  }
...
to generate control regions, as is done in the resolved
analysis. \phil{can you reference the decorrelation strategy}
...
\phil{It shocks me that resolved is better than boosted, can you add a
  few sentence explanation why?}

\phil{One note is that the word cut is jargon and should not be in
  your thesis. Overall I thought the selections part is written well,
  very concise and precise, it is easy to follow. However, I think you
  should add a diagram this would be good for your talk too.}